document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Set canvas resolution for high-quality export
    canvas.width = 1080;
    canvas.height = 1920;
...
async function renderPassport(ctx, data) {
    const W = 1080;
    const H = 1920;
    ...
    const totalIdleMinutes = Math.round(data.totalIdleHours * 60);
...
function drawStat(ctx, label, value, x, y, isBig, color = '#F8FAFC') {
    ctx.textAlign = 'center';
...
function drawBadge(ctx, label, value, icon, x, y, accentColor) {
...
function generateNarrative(data, focusMins, driftMins) {
